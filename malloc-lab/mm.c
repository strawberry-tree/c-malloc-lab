...
int mm_init(void)
{
    ...
    // (1) 메모리 시스템에서 6워드(24바이트)를 가져와서, 빈 가용 리스트를 만듦
    heap_listp = (char *)mem_sbrk(6 * WSIZE);
    ...
    PUT(heap_listp + WSIZE * 2, 0);                     // (C) 프롤로그 블록의 prev 포인터
    PUT(heap_listp + WSIZE * 3, 0);                     // (D) 프롤로그 블록의 next 포인터
